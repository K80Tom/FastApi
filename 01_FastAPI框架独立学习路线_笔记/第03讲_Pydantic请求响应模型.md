...
```text
用 Python 类型注解定义数据结构，并自动完成数据校验的工具。
```
...
```text
创建任务时，请求体里应该有 title，它必须是字符串。
description 也是字符串，但可以不传，不传时默认是空字符串。
```
...
或者缺少必填字段时，Pydantic 会校验失败，FastAPI 自动返回：
...
创建任务用它，更新任务用它，返回任务也用它。这样短期看省事，后面会很乱。
...
`BaseModel` 是 Pydantic 模型的基础类。  
继承它以后，`TaskCreate` 就不只是一个普通 Python 类，而是一个可以做数据校验、类型转换和文档生成的数据模型。
...
```text
description 可以不传，不传时默认是空字符串，最多 500 个字符。
```
...
不会把没传的 `title`、`description`、`priority` 拿出来覆盖原数据。
...
```text
这个接口返回给客户端的数据，应该整理成 TaskRead 的形状。
```
...
所以响应中不会返回 `internal_note`。  
这就是 `response_model` 很重要的能力：

```text
内部数据可以多，返回给客户端的数据要受控。
```
...
因为 `response_model=TaskRead` 已经把它过滤掉了。
...
```text
loc  -> 哪个位置出错，比如 body.title 或 query.page
msg  -> 错误说明
type -> 错误类型
```
