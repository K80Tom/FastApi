...
> 目标：先不混入 Agent、RAG、LLM 等 AI 应用内容，单独掌握 FastAPI 框架本体。  
...
- [FastAPI 学习进度](01_FastAPI框架独立学习路线_笔记/学习进度.md)
- [第 01 讲：HTTP API 基础与 FastAPI 入门](01_FastAPI框架独立学习路线_笔记/第01讲_HTTP_API基础与FastAPI入门.md)
- [第 02 讲：请求体、HTTP 方法与内存版任务 CRUD](01_FastAPI框架独立学习路线_笔记/第02讲_请求体与任务CRUD.md)
- [第 03 讲：Pydantic 请求模型与响应模型](01_FastAPI框架独立学习路线_笔记/第03讲_Pydantic请求响应模型.md)
- [第 04 讲：Pydantic 模型拆分、嵌套模型与规范响应结构](01_FastAPI框架独立学习路线_笔记/第04讲_Pydantic模型拆分嵌套与规范响应.md)
- [第 05 讲：APIRouter、项目结构与分层入门](01_FastAPI框架独立学习路线_笔记/第05讲_APIRouter项目结构与分层入门.md)
- [第 06 讲：业务异常与统一错误处理入门](01_FastAPI框架独立学习路线_笔记/第06讲_业务异常与统一错误处理入门.md)
- [第 07 讲：Depends 依赖注入入门](01_FastAPI框架独立学习路线_笔记/第07讲_Depends依赖注入入门.md)
...
一个标准任务管理后端，包含用户、项目、任务、评论、标签、鉴权、数据库、测试、Docker 部署。
...
```text
GET /tasks?page=1&page_size=20&status=doing
```
...
1. 把 TaskHub 中的普通 CRUD 经验迁移到 conversation / message / document。
2. 把文件上传能力迁移到知识库文档上传。
3. 把 StreamingResponse 迁移到 LLM 流式输出。
4. 把 BackgroundTasks / worker 迁移到文档解析和 embedding。
5. 把鉴权和 usage 统计迁移到 AI SaaS 后端。
6. 把测试经验迁移到 mock LLM、mock tool call。

换句话说：  
**FastAPI 框架本体学扎实后，AI/Agent 后端只是把业务对象换成了模型调用、文档、向量、工具和执行轨迹。**
