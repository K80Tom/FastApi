...
当客户端发送 GET /health 请求时，FastAPI 会调用 health_check 函数，函数返回的 Python dict 会被自动转换成 JSON。
...
因为函数参数写了类型：`task_id: int`，所以 FastAPI 会自动把路径里的字符串转换成整数。

如果访问 `GET /tasks/abc`，FastAPI 会返回 422 校验错误，因为 abc 不能转换成整数。
...
这里 `page=1` `page_size=20` 会自动传给函数参数 `page: int = 1` `page_size: int = 20`。
...
`/tasks/1` 含义：我要查看 id 为 1 的任务

查询参数表示筛选、分页、排序等附加条件，通常是可选的。

`/tasks?page=1&page_size=20&status=todo` 含义：我要查看任务列表，第 1 页，每页 20 条，只看 todo 状态
...
例如访问 `/tasks/abc`，而代码要求 `task_id: int`，就会触发 422。
