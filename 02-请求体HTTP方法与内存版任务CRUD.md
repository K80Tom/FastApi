...
下一步不要急着进入数据库、登录、复杂项目结构。现在最应该补齐的是：**请求体 + 常见 HTTP 方法 + 内存版 CRUD**
...
含义：请服务端创建一个新任务。客户端通常不指定最终 id，由服务端生成 id。创建成功后，常用状态码是：**201 Created**
...
含义：把 id 为 1 的任务整体替换成请求体里的新内容。PUT 更像是"完整覆盖"。如果任务有 title、description、status、priority，那请求体最好把这些字段都传完整。
...
含义：只修改 id 为 1 的任务中的部分字段。比如只把状态改成 done：
...
含义：删除 id 为 1 的任务。删除成功后，如果不需要返回内容，常用状态码是：**204 No Content**

## 5. 本讲项目目录

建议继续放在第 1 周练习目录下。

本讲建议文件：`week01_task_crud.py`
...
这里的 task_data 来自请求体。Body(...) 的意思是：这个参数从请求体中读取，并且是必填的。
...
FastAPI 会把 JSON 对象转换成 Python 字典。本讲先用 dict 接收请求体，并手动校验字段。第 2 周会把这些手动校验改成 Pydantic 模型。
...
当任务不存在时，不应该随便返回空字典，也不应该返回正常的 200 OK。更合理的是返回：**404 Not Found**
...
这些是 FastAPI 提供的状态码常量。它们本质上还是数字（201, 204, 404），用常量的好处是代码更容易读。
...
也可以带分页参数：`GET /tasks?page=1&page_size=20`

如果只看某个状态：`GET /tasks?status=todo`
...
如果任务存在，返回任务详情。如果任务不存在，返回：**404 Not Found**
...
删除成功后，再访问 `GET /tasks/1` 应该得到：**404 Not Found**

## 9. 常见错误

### 422 Validation Error

如果访问 `GET /tasks/abc`，而接口要求 `task_id: int`，FastAPI 会返回 422，因为 abc 不能转换成整数。
...
```json
{"title": "错误状态", "status": "started"}
```

status 只能是 todo/doing/done/archived，所以也返回 400。

### 404 Not Found

当任务 id 不存在时返回 404：GET /tasks/999, PUT /tasks/999, PATCH /tasks/999, DELETE /tasks/999
...
完成本讲后，第 1 周的核心目标就基本完成了：单文件 FastAPI 任务 CRUD。

下一步建议进入第 2 周：**Pydantic 与请求响应模型**
...
升级成更规范的模型：TaskCreate, TaskUpdate, TaskRead, TaskListItem, TaskDetail
